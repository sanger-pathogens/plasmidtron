...
class Options:
	def __init__(self,output_directory, file_of_traits, file_of_nontraits, verbose, threads, kmer, min_kmers_threshold,max_kmers_threshold, spades_exec, min_contig_len, action, min_spades_contig_coverage, keep_files,plot_filename, min_kmers_per_read):
...
class TestPlasmidTron(unittest.TestCase):
	...
	def test_small_valid_chrom_plasmid(self):
		...
		options = Options(os.path.join(data_dir,'out'), os.path.join(data_dir,'traits.csv'), os.path.join(data_dir,'nontraits.csv'),False, 1, 61, 20,200, 'spades.py', 100,'union', 1, False, 'plot.png', 2)
	...
	def test_small_multithreaded(self):
		...
		if os.path.exists(os.path.join(data_dir,'out')):
			shutil.rmtree(os.path.join(data_dir,'out'))
		options = Options(os.path.join(data_dir,'out'), os.path.join(data_dir,'traits.csv'), os.path.join(data_dir,'nontraits.csv'),False, 2, 61, 20,200, 'spades.py', 100,'union', 1, False, 'plot.png', 2)
	...
	def test_non_trait_fasta(self):
		...
		if os.path.exists(os.path.join(data_dir,'out')):
			shutil.rmtree(os.path.join(data_dir,'out'))
		options = Options(os.path.join(data_dir,'out'), os.path.join(data_dir,'traits.csv'), os.path.join(data_dir,'fasta_nontraits.csv'),False, 1, 61, 20,200, 'spades.py', 100,'union', 1, False, 'plot.png', 2)
