...
class TestPlotKmers(unittest.TestCase):
	...
	def test_three_samples(self):
		...
		temp_working_dir = tempfile.mkdtemp(dir=os.path.abspath(data_dir))
		p = PlotKmers([os.path.join(data_dir,'sample1.fa'), os.path.join(data_dir,'sample2.fa'), os.path.join(data_dir,'sample3.fa')], temp_working_dir, 1, 21, 255, True)

		self.assertTrue(p.generate_plot())
